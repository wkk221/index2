...
//# sourceMappingURL=es5-shim.map
 //# ie8 的填充方法?  ## slick 需要配合这些填充方法才能使用 ## ie7 的支持不是很好.
/*!
...
}(this,"undefined"!=typeof Meteor&&"undefined"!=typeof Package); //# 加载图片占位符文件
console.log("图片占位符库文件加载完毕");
//##== 配置插件的颜色
var holders=[
    {"theme":"t01","cfg":{"bg":"#C3BED4","fg":"#495a80"}},
    {"theme":"t02","cfg":{"bg":"#f17c67","fg":"#376957"}},
    {"theme":"t03","cfg":{"bg":"#FCD692","fg":"#a39f93"}},
    {"theme":"t04","cfg":{"bg":"#EAF048","fg":"#7f1874"}},
    {"theme":"t05","cfg":{"bg":"#5ED5D1","fg":"#7f1874"}},
    {"theme":"s01","cfg":{"bg":"#f0f2ef","fg":"#333333"}},
    {"theme":"s02","cfg":{"bg":"#3b77d9","fg":"#ffffff"}}
];
for(var i=0;i<holders.length;i++){
    Holder.addTheme(holders[i]["theme"],holders[i]["cfg"])
}
...
*
*
* */ //# 图片占位符文件 ## ie8 图像占位符会报错!!!!
//#倒计时组件
console.log("倒计时插件载入完毕");
function countDown(option){
    var  timer,iNow,iNew,iCd,t,config,This;
    This = this;
    t = 0;
    iCd = null;
    clearInterval( timer );
    config = {
        end:"2100/12/25 00:00:00"//#结束时间
        ,callback:null //# 回调函数
        ,target:".time" //# 输出目标 # 支持JQ 选择器
        ,regexp:"dd:hh:mm:ss"//# 正则格式
        ,d:".d"
        ,h:".h"
        ,m:".m"
        ,s:".s" //$(config.s).attr()
    ...
    if($(this).attr("data-end")){
        //console.log("检测到事件数据")
        config.end = $(this).attr("data-end"); //# 优先使用 标签内的倒计时数据!!!  数据绑定比较方便
    }

    iNew = new Date( config.end ); //#获取时间戳

    timer = setInterval(function(){
        // #开启定时器
        iNow = new Date()
        t = Math.floor((iNew-iNow)/1000); //# 计算倒计时还需要多少时间
        //# 获取当前电脑的时间
        var temp = config.regexp;
        //## 格式化时间规则 # 匹配 #判断

        iCd = [
            Math.floor(t/86400),  //天
            temp.search(/dd/gi)==-1
                ? (Math.floor((t%86400)/3600)+(Math.floor(t/86400)*24))
                : Math.floor((t%86400)/3600), //时
            Math.floor(t/60%60), //#分
            Math.floor(t%60) //#秒
        ];
        //console.log(typeof iCd[4])
        if(  t<=0 ){
            clearInterval( timer );
            config.callback && config.callback.call(This);
            //#倒计时结束
        }else{
            $(config.d).text(iCd[0]<10?"0"+iCd[0]:iCd[0]);
            $(config.h).text(iCd[1]<10?"0"+iCd[1]:iCd[1]);
            $(config.m).text(iCd[2]<10?"0"+iCd[2]:iCd[2]);
            $(config.s).text(iCd[3]<10?"0"+iCd[3]:iCd[3]);
            /*console.log(iCd) ## 补充0*/
            //console.log(iCd[3],iCd[3]<10)
            typeof jQuery == 'undefined'
                ?(config.target && config.target.nodeType==1 && (config.target.innerHTML=temp))
                :($(This).find(config.target).html(temp))
            //#
        }
    },1000)
}
...
(function () {
    /*表单互动*/
    var e_search,e_search__input,e_search__placeholder,e_search__droplist,e_search__first,
        t_droplist,t_ajax,t_hotkey,t_menu,
        n_hold__index,n_menu__index,
        d_holdkey,d_menukey
    ;
    e_search = $("[rel=js-search]"); //## 状态添加在这里
    e_search__input = e_search.find(".c-search__text"); //#表单输入框
    e_search__placeholder = e_search.find(".c-search__placeholder"); //# 文字占位
    e_search__droplist = e_search.find(".c-search__dropList"); //#下拉菜单 # 用不到
    e_search__first = e_search.find("[rel=js-searchFist]"); //# 第一个

    n_menu__index = 0; //# menu
    n_hold__index = 0; //# hold

    t_droplist = t_ajax = t_hotkey = t_menu = null; //# 定时器
    d_holdkey =  ["开局一条狗,装备全靠骗","小天鹅洗衣机","阿迪达斯羽绒服","耐克羽绒服"]; //# 占位符文本
    d_menukey =  ["年货抢先拍","抢家电神券","年货节神券","抢家电神券","海囤年货节"];


    e_search__first.text( d_menukey[n_menu__index] );
    set_menu(true);

    //# 菜单第一个定时器切换
    e_search__first.hover(function () {
        set_menu(false);
    },function () {
        set_menu(true);
    });
    ...
    e_search__input
        .on("focus",function () {
            console.log("获得焦点");
            var _ = $(this);
            clearTimeout(t_droplist);
            e_search__placeholder.css({color:"#c8c8c8"});
            has_key();
            set_hotkey(false);
        }).on("blur",function () {
        var _ = $(this);
        t_droplist = setTimeout(function () {
            e_search__placeholder.css({color:""});
            e_search.removeClass("is-input");
        },300);
        set_hotkey(true);
        if (_.val()!=""){
            set_hotkey(false);
    ...
    function has_key() {
        ...
        if( e_search__input.val() !="" ){
            e_search.addClass("is-input");
            e_search__placeholder.text("");
            getdb( e_search__input.val() ); //# 请求数据
        }else{
            e_search.removeClass("is-input");
            e_search__placeholder.text( d_holdkey[n_hold__index] );
            e_search__droplist.html(""); //# 清空数据
        }
    }
    function set_hotkey(b) {

        if(b){ //# 启动定时器
            clearInterval( t_hotkey );
            t_hotkey = setInterval(function () {
                n_hold__index++;
                n_hold__index = n_hold__index%d_holdkey.length;
                e_search__placeholder.text( d_holdkey[n_hold__index] );
            },5000)
        }else{ //# 清空定时器
            clearInterval( t_hotkey );
    ...
    function set_menu(b){
        if(b){
            t_menu = setInterval(function () {
                n_menu__index++;
                e_search__first.text( d_menukey[n_menu__index%d_menukey.length] );

            },3000);
        }else{
            clearInterval(t_menu);
        }
    }
    function f() {
        //#jsonp  回调占位
    ...
    function getdb(key) {
        var s_liformat_gala,s_liformat_li,s_liformat_colse,
            str,
            r_key,r_num
        ;
        s_liformat_gala =
            [
                '<li class="c-Alist__item c-Alist__item-first">',
                '<div class="c-Alist__match-key"><span class="c-Alist__gala">年货节!</span>{{@key}}</div>',
                '<div class="c-Alist__match-num">{{@number}}</div>',
                '</li>'
            ];//# 第一行活动
        s_liformat_li =
            [
                '<li class="c-Alist__item">',
                '<div class="c-Alist__match-key">{{@key}}</div>',
                '<div class="c-Alist__match-num">约{{@number}}个商品</div>',
                '</li>'
            ];//# 正常列表
        s_liformat_colse=
            [
                '<li class="c-Alist__item c-Alist__item-last">',
                '<div class="c-Alist__colse">关闭</div>',
                '</li>'
            ]; //# 关闭菜单样式
        ...
        $.ajax({
            type:"get",
            async:false,
            url:"http://suggest-squanqiu.jd.com/?terminal=shouquanqiu&key="+key+"&callback=f",
            dataType:"JSONP",
            jsonpCallback:"f",
            success:function (data) {
                ...
                r_key = /{{@key}}/;
                r_num = /{{@number}}/;
                str += s_liformat_gala.join("");
                for( var i=0; i<data.length; i++ ){
                    if(data[i].keyword){
                        str += s_liformat_li.join("").replace(r_key,data[i].keyword).replace(r_num,data[i].qresult)
                    }
                };
                str+= s_liformat_colse.join("");
                e_search__droplist.html( str );
            }
        })
    }
    /*表单互动end*/
...
(function () {
    /*吸顶 */
    var c_navgtion,
        n_ref,n_top,n_ref,n_templateheight,n_Y,
        b_lockstate
    ;

    c_navgtion = $("[rel=js-navgtion]");
    c_navgtion_bar = c_navgtion.find(".c-search__inner");
    n_top = c_navgtion.offset().top;
    n_templateheight = c_navgtion.height();
    n_ref = 500;
    b_lockstate = true;
    ...
    $(window).scroll(function () {
        var _ = $(this);
        n_Y = _.scrollTop();
        if( n_Y > n_templateheight + n_top + n_ref ){
            if( !b_lockstate ) return ;
            c_navgtion.addClass("is-locktop")
            c_navgtion_bar.css({top:"-48px"}).animate({top:0},600);
            b_lockstate = false;
            //console.log("触发吸顶")
        }else{
            if( b_lockstate ) return ;
            c_navgtion.removeClass("is-locktop");
            b_lockstate = true;
            //console.log("----")
        }
    }).trigger("scroll");
...
(function () {
    //# 按钮部分
    var c_menus = $("[rel=js-MenuMap]");
    var c_menu__catalog = c_menus.find(".c-Alist--MenuMap");
    var c_menu__catalog__item = c_menu__catalog.find(".c-Alist__item");
    //# 弹出层部分
    var c_menu__catalogpop = c_menus.find(".c-Alist--MenuMapPop");
    var c_menu__catalogpop__item =  c_menu__catalogpop.find(".c-Alist__item");

    //console.log(c_menus,c_menu__catalog,c_menu__catalog__item,c_menu__catalogpop,c_menu__catalogpop__item)
    var t_catalog = null;

    //## 弹出层展开逻辑
    c_menu__catalog__item.mouseover(function () {
        var index  = $(this).index();
        c_menu__catalogpop.css({display:"block"});
        c_menu__catalogpop__item.removeClass("is-open").eq(index).addClass("is-open");
    });
    ...
    $(window).scroll(function () {
        var _ = $(this);
        var n_top = c_menu__catalog.offset().top; //# 到浏览器顶部的位置
        var n_sctop = _.scrollTop(); //# 滚动条位置
        var n_height = c_menu__catalog.outerHeight(); //# 模块高度
        //c_menu__catalogpop

        //console.log(n_height);
        var b_sc = n_top - n_sctop;

        if( b_sc < 0 && b_sc > -n_height  ){

            c_menu__catalogpop.finish().animate({top:-b_sc},80);
            if(b_fg){ b_fg=false; };
            //console.log("正在计算位置");
        }else{
            if(!b_fg){
            ...
            c_menu__catalogpop.stop().css({top:"0px"});
            //console.log("我只要触发一次")
        }
    });

    //## 弹出层隐藏逻辑
    c_menus.mouseleave(function () {
        t_catalog =setTimeout(function () {
            c_menu__catalogpop.css({display:"none"});
            c_menu__catalogpop__item.removeClass("is-open");
        },600);
        console.log("启用定时器")
    }).mouseenter(function () {
        clearTimeout(t_catalog);
        //# 清空定时器
        console.log("清空定时器")
    });
...
(function () {
    //## 公告部分
    var c_news = $("[rel=js-news]");
    var c_news__hd__item = c_news.find(".Alist--CoverNewsHd .Alist__item");
    var c_news__bd__item = c_news.find(".Alist--CoverNewsBD .Alist__item");
    var c_news__bar = c_news.find(".c-news__bar");
    //#
    c_news__hd__item.mouseover(function () {
        var index = $(this).index();
        //console.log(index)
        c_news__bd__item.hide().eq(index).show(); //# 展开对应内容
        c_news__bar.finish().animate({left:(index*50)+13}); //#下划线互动
    });
})();
...
(function () {
    //alert(123);
    var e_Ranktab = $("[rel=js-tab]");
    e_Ranktab.each(function () {
        var e_hd_list = $(this).find(".c-Blist--tabHd .c-Blist__item");
        var e_bd_list = $(this).find(".c-Blist--tabBd .c-Blist__item");
        var _ = this;

        e_hd_list.on("mouseover",function () {
            //## tab 事件
            var n_index = $(this).index();
            e_hd_list.removeClass("is-active").eq(n_index).addClass("is-active");
            e_bd_list.removeClass("is-show").eq(n_index).addClass("is-show");

            ///## 刷新slick
            $(_).find("[rel=js-slick]").eq(n_index).slick("refresh");
        })
...
(function () {
    $("[rel=js-cd]").each(function () {
        countDown.call(this,{
            end:"2019/12/25 00:00:00",
            d:".c-cd__d",
            h:".c-cd__h",
            m:".c-cd__m",
            s:".c-cd__s",
